            ...
            <div key={item.id ?? item._cid}
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                  ...
                  {itemEff(item) && <span style={{fontSize:9,color:C.green}}>✨ {itemEff(item)}</span>}
        ...
        <div onClick={() => setModal(null)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
              ...
              {itemEff(modal) && <span style={sx.tag(C.green)}>✨ {itemEff(modal)}</span>}
              {modal.wt  && modal.wt!=="—" && <span style={sx.tag(C.textDim)}>⚖️ {modal.wt}</span>}
            </div>

            {itemNotes(modal) && itemNotes(modal)!=="—" && (
